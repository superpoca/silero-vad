# Silero VAD v4 / v5 / v6、TEN-VAD 与 FireRedVAD 评估协议

本文说明在 AISHELL-5 与 VoxConverse 上对比 Silero v4、v5、v6、**TEN-VAD** 以及 **FireRed Stream-VAD** 时实际采用的模型、**评了哪些数据**、语音/非语音划分，以及每一步为什么这样设计。Silero / TEN 的实现是 `eval_vad_datasets.py`；FireRed 因依赖 Kaldi fbank / CMVN / 独立 conda 环境，实现是 `eval_fireredvad_datasets.py`。数值结果见 [`eval_vad_results.md`](eval_vad_results.md)（机器可读原文 `eval_vad_results.json`）。

Silero 评估对象是**官方双速率 stock ONNX**，不是 `convert_single_rate.py` / `convert.py` 产出的 16 kHz 单速率图。TEN-VAD 用本地 `D:\ten-vad` 的 Windows x64 DLL，hop **256**（16 ms）。FireRed 用 `D:\FireRedVAD` 的流式 ONNX（带 cache），hop **160**（10 ms）。数据集范围见第 2 节：只用 AISHELL-5 的 Dev/Eval1/Eval2（近场 `DA*` + 远场 `DX01–04`）和 VoxConverse 0.3 的全部 dev+test，16 kHz。

真值规则（并集、50% 重叠）五个模型相同。帧格子随 hop 变：Silero 31.25 ms，TEN 16 ms，FireRed 10 ms。AUC / Acc / F1 因此不是同一套时间轴上的数，但仍是各自原生窗上的帧级微平均，适合看「这个模型按它自己的步长判得怎么样」。

---

## 1. 评估用模型路径

脚本里的映射（Silero 相对仓库根目录 `silero-vad/`，TEN / FireRed 为绝对路径）：
...
| firered | `D:\FireRedVAD\pretrained_models\onnx_models\fireredvad_stream_vad_with_cache.onnx` | FireRed Stream-VAD 流式 ONNX（`feat` + `caches_in`） | 160 | `b3c97836130dc34fc32d56fab551e88cf9454511de2b3c250a5e6578dee74b93` |
...
| `D:\FireRedVAD\pretrained_models\onnx_models\fireredvad_stream_vad.onnx` | 无 cache 的 Stream-VAD 图，评估未使用 |
| `D:\FireRedVAD\pretrained_models\onnx_models\fireredvad_vad.onnx` | 非流式 VAD 图，评估未使用 |
| `D:\FireRedVAD\pretrained_models\onnx_models\fireredvad_aed.onnx` | AED 图，不是帧级 VAD |
| `D:\FireRedVAD\pretrained_models\FireRedVAD\Stream-VAD\model.pth.tar` | PyTorch 权重，评估走 ONNX |

v5 与 v6 的 ONNX 体积同为 2 327 524 字节，但哈希不同，权重不是同一份。

FireRed 前端还需要同目录的 `cmvn.ark`（SHA-256 `c87f6f13edf0f0ec7535ddfc9cc3387d9268cb234b70182d566c5e2edf3ca473`，与 `pretrained_models/FireRedVAD/Stream-VAD/cmvn.ark` 相同）。特征是 Kaldi fbank 80 维、25 ms 窗 / 10 ms 移、`snip_edges=True`，再乘这份 CMVN；**不是**把 PCM 窗直接送进网络。

Silero 由 ONNX Runtime `CPUExecutionProvider` 加载；TEN-VAD 经 `D:\ten-vad\include\ten_vad.py` 加载 `ten_vad.dll`；FireRed 同样用 CPU ORT，但必须在 conda 环境 `fireredvad` 里跑（`kaldi_native_fbank` / `kaldiio` / 官方 `AudioFeat`）。每条音频按各自流式契约逐步推进内部状态（见第 5.6 节）。阈值类指标默认用 `0.5`。只评 TEN 时：`python examples/eval_vad_datasets.py --models ten`；只评 FireRed 时：`python examples/eval_fireredvad_datasets.py`。两者都会合并进已有 JSON，不覆盖已有模型键。
...
**先分清两件事：** 模型吃波形或从波形抽出的特征；0/1 标签是事后用来打分的，不进网络。

**模型输入（喂进网络）**

- 来源：16 kHz 单声道 wav
- hop 是切窗、打标、出一帧概率的步长，**不等于**网络 `input` 长度，各模型不同：
  - Silero：hop **512**（31.25 ms）。v4 只喂当前这 512 点 float32；v5/v6 前面再拼 64 点 context，一共 **576** 点（`[context 64 | hop 512]`）。
  - TEN-VAD：hop **256**（16 ms），喂当前这 256 点 **int16** PCM，不拼 context。
  - FireRed：hop **160**（10 ms）。不喂 PCM 窗；先 Kaldi fbank（25 ms 窗 / 10 ms 移）+ CMVN，再逐帧喂 80 维 `feat`。
- 完整契约见第 5.6 节。

**评估真值（不喂进网络）**

- 来源：AISHELL-5 的 TextGrid，或 VoxConverse 的 RTTM
- 内容：落到**该模型自己的 hop 格子**上的 `1` / `0`（语音 / 非语音）
...
| 切窗打标 | 见第 4.4 节：按该模型自己的 hop 切窗，重叠 ≥ 50% 标成语音 | 模型输出是帧，真值必须在同一分辨率上 |
...
实现是 `intervals_to_frames()`。一条 wav 有 **N** 个采样点（16 kHz），hop 记为 **H**。Silero H = 512，TEN H = 256，帧数

```text
F = ceil(N / H)
```

FireRed 不是按 PCM 切窗，而是官方 Kaldi fbank（25 ms 窗、10 ms 移、`snip_edges=True`）产出多少帧就评多少帧：

```text
H = 160
F = 1 + floor((N − 400) / 160)    （N < 400 则 F = 0，本批数据不会发生）
```

也就是丢掉末尾凑不齐 25 ms 分析窗的那一小段；**这段尾巴没有模型输出，也不打标签**。标签仍标在 hop 区间 `[i×10 ms, (i+1)×10 ms)` 上，与 Silero/TEN 的 50% 规则相同，只是格子更细。
...
| 窗长 | 满窗 Silero **31.25 ms**，TEN **16 ms**，FireRed **10 ms**。Silero/TEN：最后一窗若 N 不是 H 的倍数，真实音频只覆盖到 N/16000 秒，右侧用 0 填满 H 点再送给模型；**打标只用真实覆盖的时长**，不把补零算进分母。FireRed：`snip_edges` 下满 hop 都落在 wav 内，最后一窗一般无需截断 |
...
数值例子（以 Silero 满窗 31.25 ms，di = 31.25 ms 为例；TEN 满窗是 16 ms，50% 阈值按 8 ms 同比；FireRed 满窗是 10 ms，50% 阈值按 5 ms 同比）：
...
| `get_speech_timestamps` / FireRed `StreamVadPostprocessor` 的最短语音/静音/pad | 那是部署分段，不是帧标签 |
...
Silero 模型的直接输出是每窗一个概率，官方质量表也是 31.25 ms 上的 ROC-AUC / Accuracy。若先跑 `get_speech_timestamps`（滞后阈值、最短语音、最短静音、两端 pad），再拿分段去对 RTTM，测到的是「模型 + 一套超参」的部署效果，无法在同一后处理上公平对比，也无法和 wiki 对表。FireRed 官方推理链里的 `StreamVadPostprocessor` 同类，本评测同样关掉。
...
16 kHz 下 Silero 窗口固定 512，TEN 固定 256。尾块补零。Silero 与 `OnnxWrapper.audio_forward` / `get_speech_timestamps` 一致；TEN 官方 `examples/test.py` 用 `N // 256` 丢掉尾部，本评测为对齐「最后一窗也计分」而补零。FireRed 不喂 PCM 窗，见下面 **firered**。
...
- 用官方 `fireredvad.core.audio_feat.AudioFeat`：int16 PCM → Kaldi fbank（80 维、25 ms / 10 ms、`snip_edges=True`）→ `cmvn.ark`。
- ONNX 输入 `feat` `[1, 1, 80]`、`caches_in` `(8, 1, 128, 19)`；输出 `probs`、`caches_out`。逐帧推进，cache 从全零开始。
- 若导出图把 `caches_out` 的时间维写成动态，只保留末 19 帧，与 `examples/compare_onnx_stream_vad.py` 一致。
- 取原始 `probs`（0～1）进指标，**不**跑 `StreamVadPostprocessor`。
- 换文件时 cache 清零。

换文件时 Silero 的状态与 context 清零。原因：VAD 的循环状态携带历史；跨文件不重置会把上一条的说话状态泄漏到下一条。

每个进程只开 1 个 ORT 线程（TEN 则每文件一个 DLL handle），用多进程按文件并行。Silero / TEN 的 RTF 只统计模型前向，不含解码与解析标注。FireRed 的 RTF **包含** fbank + CMVN + ORT，因为没有这段前端就没有网络输入。

### 5.7 指标：先 AUC，再默认 0.5，最后才是「dev 上搜阈值」

| 指标 | 定义 | 为什么报它 |
| --- | --- | --- |
| ROC-AUC | 以概率排序，对 TPR–FPR 折线求面积 | wiki 主表；与阈值无关，适合各模型对比 |
...
最后打印 `AUC / Acc@0.5 / F1@0.5` 对照表，便于 v4 / v5 / v6 / ten / firered 并排。只跑 `--models ten` 或 `eval_fireredvad_datasets.py` 时会读入已有 JSON 再合并写入。
...
| FireRed `StreamVadPostprocessor` | 与关掉 Silero `get_speech_timestamps` 同一原因 |
...
3. `soundfile` 读 wav。Silero → `float32`；TEN / FireRed → `int16`。长度 N sample。Silero/TEN 帧数 `F = ceil(N / H)`；FireRed 帧数由 fbank `snip_edges` 决定（见第 4.4 节）。
4. 对 i = 0 … F−1：按第 4.4 节写 `labels[i] ∈ {0,1}`。
5. 按第 5.6 节流式推理，得到 `probs[i] ∈ [0,1]`。v4 不拼 context；v5/v6 拼 64 点 context；TEN 喂 256 点 int16；FireRed 先 fbank+CMVN，再逐帧喂 `[1,1,80]` 并回写 cache。
...
conda run -n fireredvad python examples/eval_fireredvad_datasets.py --limit 2 --workers 4
```

默认读取 `D:\AISHELL-5-Data` 与 `D:\voxconverse`，写出 / 合并 `examples/eval_vad_results.json`。只跑 `--models ten` 或 `eval_fireredvad_datasets.py` 时会**合并**进已有 JSON，保留已有模型键。TEN 需要 `D:\ten-vad\include\ten_vad.py` 与 `D:\ten-vad\lib\Windows\x64\ten_vad.dll`。FireRed 需要 conda 环境 `fireredvad`、`D:\FireRedVAD` 源码（`AudioFeat`）以及第 1 节的 ONNX + `cmvn.ark`。换机器时改脚本顶部的 `AISHELL_ROOT` / `VOX_ROOT` / `TEN_ROOT` / `FIRERED_ROOT`。`--limit` 按**每个数据条件**截断文件数，只用于冒烟，不能当正式分。
